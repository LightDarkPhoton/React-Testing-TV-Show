...
//import {fetchShow as mockFetchShow} from './api/fetchShow.js' if we weren't using export default for the fetchShow.js in order to use it properly
...
// Trevor Buchanan showed me we don't use an array, but an object inside of data. That seemed to make the biggest mistake I made.
const testData = { data: {
    id: 2993,
...
test("render without errors", async () => {
    ...
    // Going to wait for web page to load by checking if the dropdown menu has loaded
    ...
    // Check if episode 9 exists
    const episodeNineText = screen.getByText(/Chapter Nine: The Gate/i)
    expect(episodeNineText).toBeInTheDocument()
})